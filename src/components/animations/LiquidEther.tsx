  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container: any) {
        ...
        this.pixelRatio = Math.min((window as any).devicePixelRatio || 1, 2);
        this.resize();
    ...
    class MouseClass {
      ...
      setCoords(x: number, y: number) {
        if (!this.container) return;
        if (this.timer) (window as any).clearTimeout(this.timer);
        const rect = this.container.getBoundingClientRect();
        ...
        this.timer = (window as any).setTimeout(() => {
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          ...
          this.takeoverStartTime = (performance as any).now();
      ...
      update() {
        if (this.takeoverActive) {
          const t = ((performance as any).now() - this.takeoverStartTime) / (this.takeoverDuration * 1000);
          if (t >= 1) {
    ...
    class AutoDriver {
      ...
      lastTime = (performance as any).now();
      activationTime = 0;
      margin = 0.2;
...
`;
    ...
    class ShaderPass {
      ...
      init() {
        ...
        if (this.uniforms) {
          this.material = new (THREE as any).RawShaderMaterial(this.props.material);
    ...
    class Advection extends ShaderPass {
      ...
      createBoundary() {
        ...
        const boundaryM = new (THREE as any).RawShaderMaterial({
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps: any) {
        ...
        const mouseM = new (THREE as any).RawShaderMaterial({
    ...
    class Output {
      ...
      init() {
        ...
        this.output = new THREE.Mesh(
          ...
          new (THREE as any).RawShaderMaterial({
            vertexShader: face_vert,
    ...
    class WebGLManager {
      ...
      constructor(props: any) {
        ...
        this.lastUserInteraction = (performance as any).now();
        Mouse.onInteract = () => {
          this.lastUserInteraction = (performance as any).now();
        ...
        (window as any).addEventListener('resize', this._resize);
        this._onVisibility = () => {
      ...
      loop() {
        ...
        this.render();
        rafRef.current = (window as any).requestAnimationFrame(this._loop);
      }
      start() {
        if (this.running) return;
        this.running = true;
      ...
      dispose() {
        try {
          (window as any).removeEventListener('resize', this._resize);
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (!webglRef.current) return;
        if (isVisible && !(document as any).hidden) {
    ...
    const ro = new ResizeObserver(() => {
      if (!webglRef.current) return;
      if (resizeRafRef.current) (window as any).cancelAnimationFrame(resizeRafRef.current);
      resizeRafRef.current = (window as any).requestAnimationFrame(() => {
        if (!webglRef.current) return;
    ...
    return () => {
      if (rafRef.current) (window as any).cancelAnimationFrame(rafRef.current);
